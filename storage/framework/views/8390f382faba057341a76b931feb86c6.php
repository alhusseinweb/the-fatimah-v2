...
<?php $__env->startSection('title', "تفاصيل الفاتورة رقم " . $invoiceTitleNumberDisplay); ?>
    ...
    <div class="invoice-card">
        <div class="invoice-card-header">
            <h1 class="invoice-card-title">
                تفاصيل الفاتورة رقم <?php echo e($invoiceTitleNumberDisplay); ?>

            </h1>
            <a href="<?php echo e(route('customer.dashboard')); ?>" class="btn-action btn-back">
        ...
        <div class="invoice-card-body">
            ...
            <?php if($alertText): ?>
            <div class="alert <?php echo e($alertClass); ?> d-flex justify-content-between align-items-center mb-4 flex-wrap" role="alert">
                <span class="flex-grow-1 mb-2 mb-md-0"><?php echo e($alertText); ?></span>
                
                
                <?php if($allowTamaraPayment && $remainingAmount > 0.009): ?>
                    <div class="d-flex flex-column align-items-end">
                        <form method="POST" action="<?php echo e(route('payment_retry_tamara', $invoice->id)); ?>" class="m-0">
                            <?php echo csrf_field(); ?>
                            <button type="submit" class="btn btn-sm btn-tamara-pay" <?php echo e($remainingAmount > 3000 ? 'disabled' : ''); ?>>
                                <img src="<?php echo e(asset('images/tamara.png')); ?>" alt="Tamara">
                                <?php echo e($invoice->status == \App\Models\Invoice::STATUS_PARTIALLY_PAID ? 'ادفع المتبقي عبر تمارا' : 'ادفع الآن عبر تمارا'); ?>

                            </button>
                        </form>

                        <?php if($remainingAmount > 3000): ?>
            ...
            <div class="invoice-section">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="info-row"> <span class="info-label">رقم الفاتورة:</span> <span class="info-value invoice-number-value"><?php echo e($invoice->invoice_number ?? $invoice->id); ?></span> </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-row">
                            ...
                            <span class="info-value">
                                <span class="badge <?php echo e($invoice->status_badge_class ?? 'badge-secondary'); ?>">
                                    <?php echo e($invoice->status_label ?? Illuminate\Support\Str::ucfirst(str_replace('_', ' ', $invoice->status))); ?>
                    ...
                    <div class="col-md-6">
                        <div class="info-row"> <span class="info-label">المبلغ الإجمالي:</span> <span class="info-value fw-bold"><?php echo e(formatAmountConditionallyGlobalSafeInvoiceShow($invoice->amount, $invoice->currency)); ?></span> </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-row"> <span class="info-label">المبلغ المدفوع:</span> <span class="info-value text-success fw-bold"><?php echo e(formatAmountConditionallyGlobalSafeInvoiceShow($invoice->total_paid_amount, $invoice->currency)); ?></span> </div>
                    </div>
                    <?php if($remainingAmount > 0.009 && $invoice->status != \App\Models\Invoice::STATUS_PAID): ?>
                        <div class="col-md-6">
                            <div class="info-row"> <span class="info-label">المبلغ المتبقي:</span> <span class="info-value text-danger fw-bold"><?php echo e(formatAmountConditionallyGlobalSafeInvoiceShow($remainingAmount, $invoice->currency)); ?></span> </div>
                        </div>
                    <?php endif; ?>
                    <div class="col-md-6">
                        <div class="info-row">
                            ...
                            <span class="info-value">
                                <?php if($invoice->payment_option === 'down_payment'): ?> دفع عربون
                    ...
                    <div class="col-md-6">
                        <div class="info-row">
                            ...
                            <span class="info-value">
                                <?php if($invoice->payment_method == 'tamara'): ?> تمارا
                                <?php elseif($invoice->payment_method == 'bank_transfer'): ?> تحويل بنكي
                                <?php elseif($invoice->payment_method == 'manual_by_admin' || $invoice->payment_method == 'manual_admin_deposit' || $invoice->payment_method == 'manual_admin_full'): ?> تسجيل دفعة يدوية
                    ...
                    <div class="col-md-6">
                        <div class="info-row"> <span class="info-label">تاريخ الإنشاء:</span> <span class="info-value"><?php echo e($invoice->created_at ? toArabicDigitsGlobalSafeInvoiceShow($invoice->created_at->translatedFormat('d F Y - H:i')) : '-'); ?></span> </div>
                    </div>
                    <?php if($invoice->paid_at): ?>
                        <div class="col-md-6">
                            <div class="info-row"> <span class="info-label">تاريخ أول دفعة:</span> <span class="info-value"><?php echo e($invoice->paid_at ? toArabicDigitsGlobalSafeInvoiceShow(\Carbon\Carbon::parse($invoice->paid_at)->translatedFormat('d F Y - H:i')) : '-'); ?></span> </div>
                        </div>
                    <?php endif; ?>
                    <?php if($invoice->payment_gateway_ref): ?>
                        <div class="col-md-12">
                            <div class="info-row">
                                ...
                                <span class="info-value">
                                    <?php echo e($invoice->payment_method == 'tamara' ? 'رقم طلب تمارا: ' : 'رقم المرجع: '); ?>

                                    <?php echo e($invoice->payment_gateway_ref); ?>
            ...
            <?php if($booking = $invoice->booking): ?>
                <?php $booking->loadMissing(['service', 'addOnServices']); ?>
                <div class="invoice-section">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="info-row">
                                ...
                                <span class="info-value">
                                    <a href="<?php echo e(route('customer.bookings.index')); ?>">#<?php echo e(toArabicDigitsGlobalSafeInvoiceShow($booking->id)); ?></a>
                                </span>
                            </div>
                        </div>
                        <?php if($booking->service): ?>
                            <div class="col-md-6">
                                <div class="info-row"> <span class="info-label">الخدمة الأساسية:</span> <span class="info-value"><?php echo e($booking->service->name_ar ?? $booking->service->name_en); ?></span> </div>
                            </div>
                        <?php endif; ?>
                        <div class="col-md-6">
                            <div class="info-row"> <span class="info-label">تاريخ ووقت الحجز:</span> <span class="info-value"><?php echo e($booking->booking_datetime ? toArabicDigitsGlobalSafeInvoiceShow(\Carbon\Carbon::parse($booking->booking_datetime)->translatedFormat('l، d F Y - h:i a')) : 'غير محدد'); ?></span> </div>
                        </div>
                        
                        <div class="col-md-6">
                            <div class="info-row"> <span class="info-label">منطقة التصوير:</span> <span class="info-value"><?php echo e($booking->shooting_area_label); ?></span> </div>
                        </div>
                        <?php if($booking->shooting_area === 'outside_ahsa' && $booking->outside_location_city): ?>
                            <div class="col-md-6">
                                <div class="info-row"> <span class="info-label">المدينة (خارج الأحساء):</span> <span class="info-value"><?php echo e($booking->outside_location_city); ?></span> </div>
                            </div>
                        <?php endif; ?>
                        <?php if($booking->outside_location_fee_applied > 0): ?>
                            <div class="col-md-6">
                                <div class="info-row"> <span class="info-label">رسوم خارج المنطقة:</span> <span class="info-value"><?php echo e(formatAmountConditionallyGlobalSafeInvoiceShow($booking->outside_location_fee_applied, ($invoice->currency ?? 'SAR'))); ?></span> </div>
                            </div>
                        <?php endif; ?>
                        <div class="col-md-6">
                            <div class="info-row"> <span class="info-label">مكان الفعالية (العنوان):</span> <span class="info-value"><?php echo e($booking->event_location ?? '-'); ?></span> </div>
                        </div>
                        <div class="col-md-6">
                            <div class="info-row">
                                <span class="info-label">حالة الحجز:</span>
                                <span class="info-value">
                                    <span class="badge <?php echo e($booking->status_badge_class ?? 'bg-secondary'); ?>">
                                        <?php echo e($booking->status_label ?? Illuminate\Support\Str::ucfirst(str_replace('_', ' ', $booking->status))); ?>
                                    ...
                                    <ul class="add-on-services-details-list">
                                        ...
                                        <li>
                                            ...
                                            <span class="add-on-price">
                                                <?php echo e(formatAmountConditionallyGlobalSafeInvoiceShow($addOn->pivot->price_at_booking, $invoice->currency)); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\mustafa\.gemini\antigravity\scratch\static\the-fatimah-old\resources\views/frontend/customer/invoices/show.blade.php ENDPATH**/ ?>