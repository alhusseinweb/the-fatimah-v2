...
<?php $__env->startSection('title', 'تأكيد الحجز'); ?>
        ...
        <div class="booking-card mb-4">
            ...
            <div class="card-body booking-summary">
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-9"><?php echo e($service->getLocalizedNameAttribute()); ?></dd>
                    <dt class="col-sm-3">التاريخ والوقت:</dt>
                    <dd class="col-sm-9"><?php echo e($bookingDateTime ? toArabicDigits($bookingDateTime->translatedFormat('l, d F Y - h:i A')) : 'غير محدد'); ?></dd>
                    <dt class="col-sm-3">السعر الإجمالي:</dt>
                    <dd class="col-sm-9 booking-price" id="total_amount_display"><?php echo e($baseServicePriceFormatted); ?> ريال سعودي</dd>
                    <dt class="col-sm-3">المبلغ المطلوب للدفع الآن:</dt>
                    <dd class="col-sm-9 booking-price" id="amount_to_pay_display"><?php echo e($baseServicePriceFormatted); ?> ريال سعودي</dd>
                </dl>
            </div>
        </div>
        
        <form action="<?php echo e(route('booking.submit')); ?>" method="POST" id="booking-form">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="service_id" value="<?php echo e($service->id); ?>">
            <input type="hidden" name="date" value="<?php echo e($selectedDate); ?>">
            <input type="hidden" name="time" id="booking_time_hidden_input" value="<?php echo e($selectedTime); ?>">
            <input type="hidden" name="payment_option" id="payment_option_input" value="full">
            <input type="hidden" name="discount_code" id="discount_code_input" value="<?php echo e(old('discount_code')); ?>">
                ...
                <div class="card-body">
                    <div class="region-options">
                        <div class="region-option-item selected" data-value="inside_ahsa">
                            <input class="form-check-input" type="radio" name="shooting_area_option" id="area_inside_ahsa" value="inside_ahsa" <?php echo e(old('shooting_area_option', 'inside_ahsa') == 'inside_ahsa' ? 'checked' : ''); ?> required>
                        ...
                        <div class="region-option-item" data-value="outside_ahsa">
                            <input class="form-check-input" type="radio" name="shooting_area_option" id="area_outside_ahsa" value="outside_ahsa" <?php echo e(old('shooting_area_option') == 'outside_ahsa' ? 'checked' : ''); ?> required>
                            <label for="area_outside_ahsa" style="cursor:pointer; margin-right: 8px;">خارج الأحساء (+<?php echo e($outsideAhsaFeeFormatted); ?> ريال رسوم إضافية)</label>
                        </div>
                    </div>
                    <div id="outside_ahs_city_group" class="mt-3" style="display: <?php echo e(old('shooting_area_option') == 'outside_ahsa' ? 'block' : 'none'); ?>;">
                        <label for="outside_ahs_city" class="form-label">الرجاء اختيار المدينة (خارج الأحساء) <span class="text-danger">*</span></label>
                        <select class="form-select" name="outside_ahs_city" id="outside_ahs_city" <?php echo e(old('shooting_area_option') == 'outside_ahsa' ? 'required' : ''); ?>>
                            <option value="">-- اختر المدينة --</option>
                            <?php $__currentLoopData = $outsideAhsaCities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($value); ?>" <?php echo e(old('outside_ahs_city') == $value ? 'selected' : ''); ?>><?php echo e($label); ?></option>
            ...
            <?php if($addOnServices && $addOnServices->count() > 0): ?>
            <div class="booking-card mb-4">
                <div class="card-header"><h5 class="mb-0">خدمات إضافية (اختياري)</h5></div>
                <div class="card-body add-on-services-list">
                    <?php $__currentLoopData = $addOnServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $addOn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="add-on-service-item">
                        <input class="form-check-input add-on-checkbox" type="checkbox" name="add_on_services[]" value="<?php echo e($addOn->id); ?>" id="add_on_<?php echo e($addOn->id); ?>" data-price="<?php echo e($addOn->price); ?>" <?php echo e(is_array(old('add_on_services')) && in_array($addOn->id, old('add_on_services')) ? 'checked' : ''); ?>>
                        <label for="add_on_<?php echo e($addOn->id); ?>"><?php echo e($addOn->getLocalizedNameAttribute()); ?></label>
                        <span class="add-on-service-price me-auto"><?php echo e(formatAmountConditionallyBookingForm($addOn->price)); ?> ريال</span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="event_location" name="event_location" value="<?php echo e(old('event_location')); ?>">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="groom_name_en" class="form-label">اسم العريس (بالإنجليزية)</label>
                            <input type="text" class="form-control" id="groom_name_en" name="groom_name_en" value="<?php echo e(old('groom_name_en')); ?>">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="bride_name_en" class="form-label">اسم العروس (بالإنجليزية)</label>
                            <input type="text" class="form-control" id="bride_name_en" name="bride_name_en" value="<?php echo e(old('bride_name_en')); ?>">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="customer_notes" class="form-label">ملاحظات إضافية (اختياري)</label>
                            <textarea class="form-control" id="customer_notes" name="customer_notes" rows="3"><?php echo e(old('customer_notes')); ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="payment-options">
                        <div class="payment-option-item selected" data-value="full">
                            ...
                            <span class="ms-auto amount" id="full_payment_option_amount"><?php echo e($baseServicePriceFormatted); ?> ريال</span>
                        </div>
                        <div class="payment-option-item" data-value="down_payment">
                            ...
                            <span class="ms-auto amount" id="down_payment_option_amount"><?php echo e($downPaymentAmountBasedOnServiceFormatted); ?> ريال</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="booking-card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="payment-methods">
                        <?php if($isTamaraEnabled): ?>
                            <div class="payment-method-item <?php echo e($defaultPaymentMethod == 'tamara' ? 'selected' : ''); ?>" data-value="tamara" style="display: flex;">
                                <input class="form-check-input" type="radio" name="payment_method" id="pay_tamara" value="tamara" <?php echo e($defaultPaymentMethod == 'tamara' ? 'checked' : ''); ?> required>
                                <img src="<?php echo e(asset('images/tamara.png')); ?>" height="28" alt="Tamara" style="margin-right: 8px; max-width: 100px; vertical-align: middle;">
                        ...
                        <?php if($isPaylinkEnabled): ?>
                            <div class="payment-method-item <?php echo e($defaultPaymentMethod == 'paylink' ? 'selected' : ''); ?>" data-value="paylink" style="display: flex;">
                                <input class="form-check-input" type="radio" name="payment_method" id="pay_paylink" value="paylink" <?php echo e($defaultPaymentMethod == 'paylink' ? 'checked' : ''); ?> required>
                                <img src="<?php echo e(asset('images/paylink.png')); ?>" height="28" alt="Paylink" style="margin-right: 8px; max-width: 100px; vertical-align: middle;">
                                <label for="pay_paylink" style="cursor:pointer;" class="ms-2"> الدفع الإلكتروني (مدى، فيزا، ماستركارد) </label>
                            </div>
                        <?php endif; ?>

                        <?php if($isBankTransferEnabled): ?>
                            <div class="payment-method-item <?php echo e($defaultPaymentMethod == 'bank_transfer' ? 'selected' : ''); ?>" data-value="bank_transfer">
                                <input class="form-check-input" type="radio" name="payment_method" id="pay_bank" value="bank_transfer" <?php echo e($defaultPaymentMethod == 'bank_transfer' ? 'checked' : ''); ?> required>
                                <img src="<?php echo e(asset('images/bank-icon.jpg')); ?>" height="24" alt="Bank Transfer" style="margin-right: 8px; margin-left: 5px; vertical-align: middle;">
                                <label for="pay_bank" style="cursor:pointer;" class="ms-1"> تحويل بنكي </label>
                            </div>
                        <?php endif; ?>
                    </div>

                    <?php if(!$isTamaraEnabled && !$isBankTransferEnabled && !$isPaylinkEnabled): ?>
                    ...
                    <?php if($isBankTransferEnabled): ?>
                        <div id="bank-details" style="display: <?php echo e($defaultPaymentMethod == 'bank_transfer' ? 'block' : 'none'); ?>;">
                            <hr class="my-3">
                            ...
                            <?php if($bankAccounts && $bankAccounts->count() > 0): ?>
                                <ul class="list-unstyled mb-2 small">
                                    ...
                                    <li class="mb-3 pb-3 border-bottom">
                                        <div class="bank-item-header"><?php echo e($account->{'bank_name_' . app()->getLocale()} ?? $account->bank_name_ar); ?></div>
                                        <div class="bank-item-detail">
                                            <span class="bank-item-detail-label">اسم المستفيد:</span>
                                            <span><?php echo e($account->{'account_name_' . app()->getLocale()} ?? $account->account_name_ar); ?></span>
                                        </div>
                                        <div class="bank-item-detail">
                                            <span class="bank-item-detail-label">رقم الحساب:</span>
                                            <span dir="ltr" class="d-inline-block user-select-all" style="text-align: left;"><?php echo e($account->account_number ?? '-'); ?></span>
                                        </div>
                                        <div class="bank-item-detail">
                                            <span class="bank-item-detail-label">رقم IBAN:</span>
                                            <span dir="ltr" class="d-inline-block user-select-all" style="text-align: left;"><?php echo e($account->iban ?? '-'); ?></span>
                                        </div>
                                    </li>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </ul>
                                <p class="text-muted small mt-3">
                                    يرجى تحويل المبلغ المطلوب إلى أحد الحسابات الموضحة أعلاه وإرسال إيصال التحويل عبر الواتساب <strong dir="ltr"><?php echo e(function_exists('toArabicDigits') ? toArabicDigits(App\Models\Setting::where('key', 'contact_whatsapp')->first()->value ?? '') : ''); ?></strong> لتأكيد حجزك.
                                </p>
                            <?php else: ?>
            ...
            <div class="booking-card mb-4">
                ...
                <div class="card-body">
                    <div class="policy-box">
                        ...
                        ?>
                        <?php echo nl2br(e($policy)); ?>

                    </div>
                    <div class="policy-check-container">
                        <input class="policy-check-input form-check-input" type="checkbox" value="1" id="agreed_to_policy" name="agreed_to_policy" <?php echo e(old('agreed_to_policy') ? 'checked' : ''); ?> required>
            ...
            <div class="d-grid gap-3 mt-4">
                ...
                <a href="<?php echo e(route('booking.calendar', $service->id)); ?>" class="btn custom-btn custom-btn-outline">
                    العودة للتقويم
                </a>
            </div>
        </form>
...
<?php if($isBankTransferEnabled && $enableBankTransferDiscountPopup && !empty($bankTransferDiscountCode)): ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
            ...
            <div class="modal-body text-center p-4">
                <p class="lead mb-3"><?php echo nl2br(e($bankTransferPopupMessage)); ?></p>
                <p class="mb-3">رمز الخصم: <strong class="text-primary" dir="ltr"><?php echo e($bankTransferDiscountCode); ?></strong></p>
...
<script>
    ...
    const baseServicePriceJS = <?php echo e((float)($baseServicePrice)); ?>;
    const outsideAhsaFeeConstJS = <?php echo e((float)($outsideAhsaFeeFromSettings)); ?>;
    ...
    const serviceIdForDiscountJS = '<?php echo e($service->id); ?>';
    const bookingTimeForDiscountJS = document.getElementById('booking_time_hidden_input')?.value || '<?php echo e($selectedTime); ?>';
    ...
    const enableBankTransferDiscountPopupJS = <?php echo e($enableBankTransferDiscountPopup ? 'true' : 'false'); ?>;
    const bankTransferDiscountCodeJS = "<?php echo e($bankTransferDiscountCode ?? ''); ?>";
    ...
    async function fetchAvailableDiscounts(paymentMethod) {
        ...
        try {
            const response = await fetch('<?php echo e(route("api.discounts.get-available")); ?>', {
                method: 'POST',
    ...
    function checkDiscountFunctionalityJS(code) {
        ...
        fetch('<?php echo e(route("api.discount.check")); ?>', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfTokenJS, },
    ...
    function populateConfirmationModal() {
        document.getElementById('modal-booking-datetime').textContent = '<?php echo e($bookingDateTime ? toArabicDigits($bookingDateTime->translatedFormat("l، d F Y - h:i a")) : ""); ?>';
        let servicesHtml = `<div class="sub-item"><span><?php echo e(e($service->getLocalizedNameAttribute())); ?></span><span>${formatDisplayAmountJS(baseServicePriceJS)} ريال</span></div>`;
        addOnCheckboxes.forEach(checkbox => {
            if (checkbox.checked) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const initialPaymentOption = "<?php echo e(old('payment_option', 'full')); ?>";
        selectPaymentOptionJS(initialPaymentOption);

        const initialPaymentMethodValue = "<?php echo e(old('payment_method')); ?>";
        let defaultInitialMethod = '';
        if (initialPaymentMethodValue) {
            if ( (initialPaymentMethodValue === 'tamara' && <?php echo e($isTamaraEnabled ? 'true' : 'false'); ?>) ||
                 (initialPaymentMethodValue === 'bank_transfer' && <?php echo e($isBankTransferEnabled ? 'true' : 'false'); ?>) ) {
                defaultInitialMethod = initialPaymentMethodValue;
            }
        }
        
        if (!defaultInitialMethod) {
            if (<?php echo e($isTamaraEnabled ? 'true' : 'false'); ?>) { defaultInitialMethod = 'tamara'; } 
            else if (<?php echo e($isBankTransferEnabled ? 'true' : 'false'); ?>) { defaultInitialMethod = 'bank_transfer'; }
        }
        if (defaultInitialMethod) { 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\mustafa\.gemini\antigravity\scratch\static\the-fatimah-old\resources\views/frontend/booking/form.blade.php ENDPATH**/ ?>