...
use Carbon\Carbon;
use Throwable;
use Illuminate\Support\Str; // *** إضافة Str هنا إذا لم يكن مستورداً ***
use Tamara\Client;
...
class PaymentController extends Controller
{
    ...
    public function handleTamaraSuccess(Request $request, Invoice $invoice): RedirectResponse
    {
        ...
        // استخدام الثوابت من موديل Invoice
    ...
    public function handleTamaraFailure(Request $request, Invoice $invoice): RedirectResponse
    {
        ...
        if (!in_array($originalStatus, [Invoice::STATUS_PAID, Invoice::STATUS_PARTIALLY_PAID])) {
            // تم تغيير منطق تحديث الحالة ليعتمد على الـ webhook بشكل أساسي
            // $invoice->status = Invoice::STATUS_FAILED;
            // $invoice->save();
            Log::info("Invoice status marked as FAILED due to Tamara failure redirect (will be confirmed by webhook).", ['invoice_id' => $invoice->id]);
    ...
    public function handleTamaraWebhook(Request $request)
    {
        ...
        if (empty($notificationKey)) {
            ...
            return response()->json(['status' => 'error_config'], 500);
        }
        $eventType = null; $tamaraOrderId = null; $orderReferenceId = null; $payloadData = null;

        try {
            $notificationService = NotificationService::create($notificationKey);
            $webhookMessage = $notificationService->processWebhook(); // هذا قد يرمي استثناء إذا فشل التحقق
            Log::info('Tamara webhook processed and authenticated successfully.');
            $eventType = method_exists($webhookMessage, 'getEventType') ? $webhookMessage->getEventType() : null;
            $tamaraOrderId = method_exists($webhookMessage, 'getOrderId') ? $webhookMessage->getOrderId() : null;
            $orderReferenceId = method_exists($webhookMessage, 'getOrderReferenceId') ? $webhookMessage->getOrderReferenceId() : null;
            $payloadData = method_exists($webhookMessage, 'getData') ? $webhookMessage->getData() : [];
            Log::debug('Extracted Webhook Data:', ['event_type' => $eventType, 'order_id' => $tamaraOrderId, 'order_reference_id' => $orderReferenceId]);
            if (empty($eventType) || empty($tamaraOrderId) || empty($orderReferenceId)) {
                Log::error('Tamara Webhook Error: Missing essential data from webhook message.');
                throw new NotificationException('Missing essential data from webhook.');
            }
        } catch (ForbiddenException $e) {
            Log::error('Tamara Webhook Auth Failed.', ['msg' => $e->getMessage()]);
            return response()->json(['status' => 'error_auth'], 403);
        } catch (NotificationException | RequestException $e) { // RequestException لـ SDK v2+
            Log::error('Tamara Webhook Proc Error.', ['msg' => $e->getMessage()]);
            return response()->json(['status' => 'error_processing'], 400);
        } catch (Throwable $e) {
            Log::error('Tamara Webhook General Error.', ['msg' => $e->getMessage(), 'trace' => Str::limit($e->getTraceAsString(), 500)]);
            return response()->json(['status' => 'error_general'], 500);
        }
            ...
            if ($eventType === 'order_approved') {
                ...
                DB::transaction(function () use ($tamaraOrderId, $orderReferenceId, $payloadData) {
                    ...
                    if ($tamaraAmount !== null) {
                        ...
                        Log::warning("Could not extract amount from Tamara webhook payload for approved order. Estimating.", ['order_id' => $tamaraOrderId]);
                        // Estimating logic
                ...
                Log::info("Received unhandled Tamara webhook event type: {$eventType}", ['order_id' => $tamaraOrderId]);
            }
            return response()->json(['status' => 'success'], 200);
        } catch (Throwable $e) {
            Log::error('Unhandled exception after webhook processing.', [
    ...
    public function retryTamaraPayment(Request $request, Invoice $invoice): RedirectResponse
    {
        ...
        $allowedRetryStatuses = [
            ...
            Invoice::STATUS_PARTIALLY_PAID,
            Invoice::STATUS_PENDING // إضافة الحالة المعلقة هنا
        ];
        ...
        try {
            $tamaraService = resolve(TamaraService::class);
            // استخدام الثوابت من موديل Invoice
            ...
            Log::error('Exception during Tamara retry payment initiation.', ['invoice_id' => $invoice->id, 'error' => $e->getMessage(), 'trace' => Str::limit($e->getTraceAsString(),500)]);
