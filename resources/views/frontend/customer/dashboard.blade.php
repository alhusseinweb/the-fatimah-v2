...
<div class="dashboard-wrapper">
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12">
                {{-- بطاقة الترحيب ومعلومات الحساب --}}
                <div class="welcome-card">
                    {{-- <div class="welcome-icon"><i class="fas fa-user-circle"></i></div> --}}
                    <div class="welcome-text">
                        <h2 class="welcome-title">مرحباً بك، {{ auth()->user()->name ?? 'عميلنا العزيز' }}!</h2>
                ...
                <div class="customer-info">
                    ...
                    <div class="info-list">
                        <div class="info-item"> <div class="info-label">الاسم:</div> <div class="info-value">{{ auth()->user()->name ?? 'غير محدد' }}</div> </div>
                        <div class="info-item"> <div class="info-label">البريد الإلكتروني:</div> <div class="info-value">{{ auth()->user()->email ?? 'غير محدد' }}</div> </div>
                        <div class="info-item"> <div class="info-label">رقم الجوال:</div> <div class="info-value" dir="ltr">{{ toArabicDigits(auth()->user()->mobile_number ?? 'غير محدد') }}</div> </div>
                        <div class="info-item"> <div class="info-label">تاريخ التسجيل:</div> <div class="info-value">{{ auth()->user()->created_at ? toArabicDigits(\Carbon\Carbon::parse(auth()->user()->created_at)->translatedFormat('d F Y')) : 'غير محدد' }}</div> </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 mb-4">
                        <div class="data-card"> {{-- استخدام data-card كحاوية للعنوان والبطاقات --}}
                            ...
                            <div class="data-card-content p-0"> {{-- إزالة padding من هنا --}}
                                @if(isset($upcomingBookings) && $upcomingBookings->count() > 0)
                                    <div class="item-cards-list p-3"> {{-- إضافة padding هنا للبطاقات --}}
                                        @foreach($upcomingBookings as $booking)
                                            <div class="card item-card mb-3">
                                                <div class="card-body">
                                                    <div class="item-card-row">
                                                        <div class="item-details">
                                                            <div class="item-title">{{ $booking->service?->name_ar ?? 'N/A' }}</div>
                                                            <div class="item-subtitle">
                                                                @if($booking->booking_datetime)
                                                                    {{ toArabicDigits(\Carbon\Carbon::parse($booking->booking_datetime)->translatedFormat('l, d F Y - H:i')) }}
                                                                    <small class="text-muted d-block">({{ \Carbon\Carbon::parse($booking->booking_datetime)->diffForHumans() }})</small>
                                                                @else
                                                                    N/A
                                                                @endif
                                                            </div>
                                                        ...
                                                        <div class="item-status">
                                                            <span class="badge {{ $booking->status_badge_class ?? 'badge-secondary' }}">
                                                                {{ $booking->status_label ?? Str::ucfirst($booking->status) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                     <hr class="my-2"> {{-- فاصل اختياري --}}
                                                    <div class="item-actions justify-content-end"> {{-- محاذاة الزر لليسار --}}
                                                        <a href="{{ route('booking.pending', $booking->id) }}" class="btn-action btn-details" title="عرض التفاصيل">
                                                             عرض التفاصيل
                                                        </a>
                                                    </div>
                                                </div>
                                ...
                                @else
                                    <div class="empty-message m-3"> {{-- إضافة margin للرسالة --}}
                ...
                {{-- ============================================= --}}
                <div class="data-card"> {{-- No mb-4 needed if it's the last element before page-actions --}}
                    <div class="data-card-header">
                    ...
                    <div class="data-card-content p-0">
                        @if(isset($bookingHistory) && $bookingHistory->count() > 0)
                                    ...
                                    <div class="card item-card mb-3">
                                        <div class="card-body">
                                            <div class="item-card-row">
                                                ...
                                                <div class="item-status">
                                                     <span class="badge {{ $booking->status_badge_class ?? 'badge-secondary' }} me-1">
                                                        حالة الحجز: {{ $booking->status_label ?? Str::ucfirst($booking->status) }}
                                                     </span>
                                                     @if($booking->invoice)
                                                        <span class="badge {{ $booking->invoice->status_badge_class ?? 'badge-secondary' }}">
                                                            الفاتورة: {{ $booking->invoice->status_label ?? Str::ucfirst($booking->invoice->status) }}
                                                        </span>
                                                     @else
                                                        {{-- <span class="badge bg-light text-dark">لا توجد فاتورة</span> --}}
                                                     @endif
                                                </div>
                                            </div>
                                             <hr class="my-2">
                                             <div class="item-actions justify-content-end">
                                                 @if($booking->invoice)
                                                    <a href="{{ route('customer.invoices.show', $booking->invoice->id) }}" class="btn-action btn-invoice" title="عرض الفاتورة">
                                                         عرض الفاتورة
                                                    </a>
                                                 @endif
                                                 <a href="{{ route('booking.pending', $booking->id) }}" class="btn-action btn-details" title="عرض التفاصيل">
                             ...
                             {{-- <div class="p-3"> {{ $bookingHistory->links() }} </div> --}}
                        @else
                            <div class="empty-message m-3">
                ...
                <div class="page-actions">
                    <a href="{{ route('services.index') }}" class="btn-page-action btn-primary-action"> تصفح خدمات التصوير </a>
