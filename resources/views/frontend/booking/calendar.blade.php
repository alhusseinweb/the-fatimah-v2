{{-- resources/views/frontend/booking/calendar.blade.php --}}
@extends('layouts.app')

@section('title', 'اختيار موعد الحجز لـ ' . ($service->{'name_' . app()->getLocale()} ?? $service->name_ar))
...
<div class="container py-4">
    ...
    <div class="service-info mb-4">
        <h1 class="h5 mb-3">{{ $service->{'name_' . app()->getLocale()} ?? $service->name_ar }}</h1>
        <div class="info-item"><span>المدة: {{ toArabicDigits($service->duration_hours ?? '0') }} ساعات</span> </div>
        <div class="info-item"><span>السعر: <span class="service-price-value">{{ toArabicDigits(number_format($service->price_sar, 0)) }}</span> ريال</span> </div>
    </div>
    ...
    <div class="mobile-calendar-wrapper">
        ...
        <div class="days-grid" id="days-grid"> {{-- سيتم ملء الأيام بواسطة JavaScript --}} </div>
    </div>

    {{-- رسائل المعلومات والأخطاء --}}
    ...
    <div class="time-slots-container" id="time-slots-container" style="display: none;">
        ...
        <div class="time-slots-grid" id="time-slots-grid"> {{-- سيتم ملء الأوقات بواسطة JavaScript --}} </div>
        <div id="no-slots-message" class="alert alert-warning text-center py-3" style="display: none;"> لا توجد أوقات متاحة لهذا اليوم. </div>
        <div class="confirm-btn-container" id="confirm-btn-container"> {{-- ** إضافة ID هنا ** --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const photographerWhatsAppNumber = '{{ $photographerWhatsApp ?? '' }}';
        const maxBookingMonthsValue = '{{ \App\Models\Setting::where('key', 'months_available_for_booking')->value('value') }}';
        ...
        if(continueBtn) continueBtn.addEventListener('click', function() { if (!this.disabled && selectedDate && selectedTimeSlot) { const dateStr = formatDate(selectedDate); window.location.href = `{{ route('booking.showForm') }}?service_id=${serviceId}&date=${dateStr}&time=${selectedTimeSlot}`; } });
