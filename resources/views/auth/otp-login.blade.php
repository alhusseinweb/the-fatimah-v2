...
<head>
    ...
    <title>{{ config('app.name', 'Fatimah Booking') }} - تسجيل الدخول</title>
...
<body>
    <section class="login-section">
        <div class="login-container">
            ...
            {{-- نموذج تسجيل الدخول --}}
            <form method="POST" action="{{ route('login.otp.request') }}" id="otpLoginForm">
                @csrf
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control @error('mobile_number') is-invalid @enderror"
                           ...
                           pattern="^05\d{8}$" {{-- يمكنك إزالة هذا النمط إذا كنت تعتمد فقط على التحقق من الخادم --}}
                           required autofocus>
    ...
    {{-- ============================================================= --}}
    {{-- كود مربع الحوار (المودال) الخاص بعدم وجود حساب --}}
    {{-- ============================================================= --}}
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            ...
            {{-- حقل مخفي لتخزين رقم الجوال لاستخدامه عند الانتقال لصفحة التسجيل --}}
            <input type="hidden" id="modalMobileNumber">
          </div>
          <div class="modal-footer justify-content-center">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">لا</button>
            {{-- زر "نعم" لإنشاء حساب جديد --}}
    ...
    {{-- ============================================================= --}}
    {{-- كود JavaScript لمعالجة إرسال النموذج وعرض المودال وتعطيل الزر --}}
    {{-- ============================================================= --}}
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (loginForm && mobileNumberInput && submitButton && noAccountModalElement && noAccountModal && modalMobileNumberInput && confirmCreateAccountButton) {
                    ...
                    if(response.ok || response.redirected) {
                        ...
                        if (response.status === 200) {
                            return response.json().then(data => {
                                ...
                                window.location.href = "{{ route('login.otp.verify.form') }}?mobile_number=" + encodeURIComponent(mobileNumber);
            ...
            confirmCreateAccountButton.addEventListener('click', function () {
                 ...
                 window.location.href = "{{ route('register') }}?mobile_number=" + encodeURIComponent(mobileNumber);
             });
